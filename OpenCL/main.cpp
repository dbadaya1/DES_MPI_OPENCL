...
string CBCEncrypt(const string &plaintext) {
	int rem = plaintext.length() % 8;
	int originalLength = plaintext.length();
	int length;
	char *text;
	if (rem) {
		length = originalLength + 8 - rem;
		text = new char[length];
		for (int i = 0; i < 8 - rem; i++) {
			text[originalLength + i] = 0;
		}
	}
	else {
...
int main() {
	...
	generateSubKeys(true);

	cipher = encrypt(text);

	generateSubKeys(false);
