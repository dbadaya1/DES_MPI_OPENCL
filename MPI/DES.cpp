...
string execute(const string &text,bool isEncryption) {
...
/*
string decrypt(const char cipher[8]) {
	int binary[64];

	char buffer[8];
	int k = 0;
	for (int i = 0; i < 8; i++) {
		std::bitset<8> x(cipher[i]);
		for (int j = 0; j < 8; j++) {
			binary[k++] = x[7 - j] ? 1 : 0;
		}
	}
	...
	for (int round = 1; round <= 16; round++) {
		...
		F(R[round - 1], subKeys[17 - round], temp);
		xor32(L[round - 1], temp, R[round]);
	}
	int RL[64];
	for (int i = 0; i < 32; i++) {
