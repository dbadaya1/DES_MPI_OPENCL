...
void execute(char text[8], int subKeys[17][48],char cipher[8]) {

	int binary[64];

	for (int i = 0, k = 0; i < 8; i++) {
		for (int j = 7; j >= 0; --j, text[i] >>= 1) {
			binary[k + j] = (text[i] & 1);
		}
		k += 8;
	}
...
void encrypt(char plaintext[8], int subKeys[17][48],char cipher[8]) {
	execute(plaintext, subKeys,cipher);
}

void decrypt(char cipher[8], int subKeys[17][48], char plaintext[8]) {
	execute(cipher, subKeys,plaintext);
}
